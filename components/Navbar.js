...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-12 sm:px-16 lg:px-20 flex justify-between items-center h-20">
        <ScrollLink
          to="hero"
          smooth={true}
          duration={500}
          offset={-80}
          className="text-2xl font-bold font-mono text-[#529d7c] cursor-pointer hover:opacity-80 transition-opacity"
        >
          {personalInfo.name}
        </ScrollLink>
        ...
        <div className="md:hidden">
          <button onClick={() => setIsOpen(!isOpen)} className="text-[#529d7c] focus:outline-none">
      ...
      {isOpen && (
        <motion.div
          ...
          className="md:hidden bg-gradient-to-b from-[#0f172a] to-[#1e293b] py-2"
        >
          {navItems.map((item) => (
            <ScrollLink
              key={item.name}
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="block text-center mt-2 mx-4 px-4 py-3 font-mono border-2 border-[#529d7c] text-[#529d7c] rounded hover:bg-[#529d7c] hover:text-[#0f172a] transition-all duration-300"
            onClick={() => setIsOpen(false)}
          >
            Resume
          </a>
